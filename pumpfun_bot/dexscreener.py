...
pair's 1h price change. A brand-new token may have no pair indexed yet;
...
async def fetch_price_change_1h_pct(mint: str, timeout_sec: float = 5.0) -> float | None:
    """Returns the 1h price change percentage for the mint's highest-
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                price_change = (pairs[0].get("priceChange") or {}).get("h1")
                return float(price_change) if price_change is not None else None
    except Exception as exc:  # noqa: BLE001
        logger.debug("Kon 1h prijsverandering niet ophalen voor %s: %s", mint, exc)
        return None
